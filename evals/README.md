...
一个测试专门展示限制：只要元数据合法，脚本并不能知道自然语言主张是否真实。因此 **26/26 通过不等于反幻觉率100%，更不等于论文写作质量已得到验证**。上述确定性测试不调用 LLM，也没有训练/复现实验。

## 2.0.2 开篇规则冒烟检查

2026-09-22 用原生子代理执行了 [五个合成案例](opening-section-cases.md)，保留 [实际输出与审查记录](opening-smoke-2026-09-22.md)。仅检查本次输入中的操作、公式、限定与证据等级是否保留，不是独立盲评或系统性质量对比；下述 A/B 评估仍未完成。
