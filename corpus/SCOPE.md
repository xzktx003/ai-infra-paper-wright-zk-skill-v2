...
2026-09-22 补核六篇开篇并将 Olica 升为 B 级，未扩大原录用样本；阅读记录见 [开篇实例](../references/opening-sections-examples.md)。当前 **42 篇主会录用记录，22 篇 B 级结构卡，20 篇 A 级摘要入口**。正文可访问性使结构样本存在偏差，尤其 ICML 2025 的结构细读仅覆盖 FlatQuant 和 Olica；不能将其代表整个 ICML。没有记录声称完成全文全部证明审查或实验复现。
...
有些官方 PDF 过大、超时或以不可读取的附件格式返回。遇到这种情况，保留 A 标签；可读取作者 arXiv HTML 时，将录用验证和结构来源分开。初次 FlatQuant 使用 v4（2025-08-10），QERA、Quamba 使用未固定 revision 的 HTML。2026-09-22 已用正式 PDF 补核 FlatQuant 与 QERA 的开篇，原记录其余部分仍保留各自阅读范围；Quamba 的版本限制未解除。不得把开篇复核当成整篇 camera-ready 核验。
